...
from api.scalars.provider_scalar import (
    Provider,
    ProviderDeleted,
    ProviderNotFound,
    ProviderExists,
...
async def get_providers(info):
    ...
    for provider in db_providers:
        ...
        provider_dict["schedules"] = (
            await s.execute(
                text(
                    select(schedule_model)
                    .where(schedule_model.provider_id == provider_dict["id"])
                    .__str__()
                ),
                {"provider_id_1": provider_dict["id"]},
            )
        ).all()
...
async def get_provider(provider_id, info):
    ...
    async with get_session() as s:
        sql = text(
            select(provider_model)
            .options(load_only(*selected_fields))
            .filter(provider_model.id == provider_id)
            .order_by(provider_model.name)
            .__str__()
        )
...
async def add_provider(name):
    ...
    async with get_session() as s:
        sql = text(select(provider_model).filter(provider_model.name == name).__str__())
        ...
        sql = text(
            select(provider_model)
            .options(load_only(getattr(provider_model, "name")))
            .filter(provider_model.name == name)
            .__str__()
        )
...
async def delete_provider(provider_id):
    ...
    async with get_session() as s:
        sql = text(
            select(provider_model).where(provider_model.id == provider_id).__str__()
        )
