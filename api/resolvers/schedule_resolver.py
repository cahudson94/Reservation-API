...
async def get_schedules(info, provider_id):
    ...
    async with get_session() as s:
        sql = (
            select(schedule_model)
            .options(load_only(*selected_fields))
            .order_by(schedule_model.start)
        )
        if provider_id:
        ...
        for schedule in db_schedules:
            ...
            schedule_dict["appointments"] = (
                await s.execute(
                    text(
                        select(appointment_model)
                        .where(appointment_model.schedule_id == schedule_dict["id"])
                        .__str__()
                    ),
                    {"schedule_id_1": schedule_dict["id"]},
                )
            ).all()
...
async def get_schedule(schedule_id, info):
    ...
    async with get_session() as s:
        sql = text(
            select(schedule_model)
            .options(load_only(*selected_fields))
            .filter(schedule_model.id == schedule_id)
            .order_by(schedule_model.start)
            .__str__()
        )
...
async def add_schedule(provider_id, start, end):
    ...
    async with get_session() as s:
        query = insert(schedule_model).values(
        ...
        sql = text(
            ...
            .options(
                load_only(
                    getattr(schedule_model, "start"), getattr(schedule_model, "end")
                )
            )
            .filter(schedule_model.provider_id == provider_id)
            .__str__()
        )
        db_schedule = (
            await s.execute(
...
async def update_schedule(schedule_id, start, end):
    ...
    async with get_session() as s:
        sql = text(
            select(schedule_model).where(schedule_model.id == schedule_id).__str__()
        )
        existing_db_schedule = (await s.execute(sql)).first()
        if existing_db_schedule is None:
            return ScheduleNotFound()

        query = delete(appointment_model).where(
            appointment_model.schedule_id == schedule_id
        )
        await s.execute(query)
        query = (
            update(schedule_model)
            .where(schedule_model.id == schedule_id)
