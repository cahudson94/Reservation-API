...
from api.scalars.client_scalar import AddClient, Client, ClientDeleted, ClientNotFound, ClientExists
...
async def get_clients(info):
	...
	async with get_session() as s:
		sql = text(select(client_model).options(load_only(*selected_fields)).order_by(client_model.name).__str__())
		db_clients = (await s.execute(sql)).all()

	clients_data_list = []
	for client in db_clients:
		client_dict = get_valid_data(client, client_model)
		client_dict["appointments"] = (await s.execute(text(select(appointment_model).where(appointment_model.client_id == client_dict["id"]).__str__()), {"client_id_1": client_dict["id"]})).all()
...
async def get_client(client_id, info):
	...
	selected_fields = get_only_selected_fields(client_model, info)
	async with get_session() as s:
		sql = text(select(client_model).options(load_only(*selected_fields)).filter(client_model.id == client_id) \
		.order_by(client_model.name).__str__())
...
async def add_client(name):
	...
	async with get_session() as s:
		sql = text(select(client_model) \
			.filter(client_model.name == name).__str__())
		...
		sql = text(select(client_model).options(load_only(getattr(client_model, 'name'))).filter(client_model.name == name).__str__())
