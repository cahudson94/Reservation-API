...
class Client(Base):
    ...
    created_datetime: DateTime = Column(DateTime, nullable=False, server_default=func.now())
    name: Optional[str] = Column(String, nullable=True)

    appointments = relationship("Appointment", cascade="all, delete", passive_deletes=True)
...
class Schedule(Base):
    ...
    id: UUID = Column(UUID(as_uuid=True), primary_key=True, index=True, default=uuid4)
    provider_id: Optional[UUID] = Column(UUID(as_uuid=True), ForeignKey(Provider.id, ondelete='CASCADE'), nullable=False)
    created_datetime: DateTime = Column(DateTime, nullable=False, server_default=func.now())
...
class Appointment(Base):
    ...
    id: UUID = Column(UUID(as_uuid=True), primary_key=True, index=True, default=uuid4)
    client_id: Optional[UUID] = Column(UUID(as_uuid=True), ForeignKey(Client.id, ondelete='CASCADE'), nullable=True)
    provider_id: UUID = Column(UUID(as_uuid=True), ForeignKey(Provider.id, ondelete='CASCADE'), nullable=False)
    schedule_id: UUID = Column(UUID(as_uuid=True), ForeignKey(Schedule.id, ondelete='CASCADE'), nullable=False)
    created_datetime: DateTime = Column(DateTime, nullable=False, server_default=func.now())
